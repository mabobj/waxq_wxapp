package com.wxapp.frame.dao;

import com.wxapp.frame.bean.ObjectMap;
import com.wxapp.frame.bean.PageBean;
import com.wxapp.frame.bean.SaasSort;
import com.wxapp.frame.util.SQLUtil;
...
public class JdbcDao {
    ...
    public int getTotalCount(String sql, List<Object> paramlist) {
        ...
        if (paramlist != null && ((List<Object>) paramlist).size() > 0)
            count = jdbcTemplate.queryForInt(countHQL, paramlist.toArray());
        else
            count = jdbcTemplate.queryForInt(countHQL);
        return count;
    }
    ...
    public int saveOrUpdate(Map<String, Object> o, String tableName) {
        ...
        return jdbcTemplate.update(sb.toString(), valueList.toArray());
    }
    ...
    public int saveBackId(Map<String, Object> o, String tableName) {
        ...
        int ri = jdbcTemplate.update(new PreparedStatementCreator() {
            @Override
    ...
    public List<?> queryByParameter(Map<String, ?> p, String tableName, String selectSql, String orderBySql) {
        ...
        if (orderBySql != null)
            sql += " " + orderBySql;
        return jdbcTemplate.queryForList(sql, valueList.toArray());
    }

    public Map<String, Object> queryById(String tableType, String id) {
        List<Map<String, Object>> queryForList = jdbcTemplate.queryForList("select * from " + tableType + " where id=?", id);
    ...
    public void deleteById(String t, String id) {
        jdbcTemplate.update("DELETE " + t + " t WHERE t.id=?", id);
    }
